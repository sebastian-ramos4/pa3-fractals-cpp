...
void ofApp::draw() {
    ...
    case '1': {
        ...
        angle += 0.01;
        drawMode1(ofGetWidth() / 2, ofGetHeight() / 2, r, depth);
    } break;
    case '2': {
        // Tree
        if(depth > 14) {depth = 14;}
        float length = 0.31 * ofGetHeight();
        drawMode2(ofGetWidth() / 2, ofGetHeight() - 20, depth, length, 1.5 * PI);
        drawMode2(ofGetWidth() / 6, ofGetHeight() - 20, depth, length / 4, 1.5 * PI);
        drawMode2(ofGetWidth() * 5 / 6 , ofGetHeight() - 20, depth, length / 4, 1.5 * PI);
    } break;
    case '3': {
        // Sierpinski Triangle
        if(depth > 10) {depth = 10;}
        float size = 0.88 * ofGetHeight();
        drawMode3((ofGetWidth() - size) / 2, ofGetHeight() / 2 - 0.4 * size, size, depth);
    } break;
    case '4':
        // Barnsley Fern
        if(depth > 50) {depth = 50;}
        drawMode4(0, 0, depth * 1000);
        break;
    case '5':
        // Koch SnowFlake
        SnowFlake().draw();
    ...
    case '6': 
        // Circle Rectangle
        drawMode6(ofGetWidth() / 2, ofGetHeight() / 2, ofGetWidth() / 4, depth);
        break;
...
void ofApp::drawMode1(float x, float y, float r, int n) {
    if (n == 0) {
        ...
        ofSetColor(ofColor( 0,255,0));} //verde
    else if(n <= 4) {
        ofSetColor(ofColor( 0,0,255));} //azul
    else if(n <= 6) {
        ofSetColor(ofColor( 255,0,0));} //rojo
...
void ofApp::drawMode6(float x, float y, float r, int n){
if(n == 0) {return;}
else if(n <= 6){
    ofSetColor(ofColor::blue);
}
...
float x2 = r * sin(0.785);
float y2 = r * cos(0.785);

ofDrawCircle(x , y , r);
ofDrawRectangle(x - x2 , y - y2 , x2 * 2, y2 * 2);

drawMode6(x , y , (x2 * 2) / 2 , n - 1);
